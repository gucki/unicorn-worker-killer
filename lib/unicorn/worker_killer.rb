  ...
  module Oom
    ...
    def self.new(app, memory_limit_min = (1024**3), memory_limit_max = (2*(1024**3)), check_cycle = 16, verbose = false)
      ObjectSpace.each_object(Unicorn::HttpServer) do |s|
        ...
        s.instance_variable_set(:@_worker_killer_oom_verbose, verbose)
      end
    ...
    def process_client(client)
      ...
      if @_worker_check_count % @_worker_check_cycle == 0
        ...
        logger.info "#{self}: worker (pid: #{Process.pid}) using #{rss} bytes." if @_worker_killer_oom_verbose
  ...
  module MaxRequests
    ...
    def self.new(app, max_requests_min = 3072, max_requests_max = 4096, verbose = false)
      ObjectSpace.each_object(Unicorn::HttpServer) do |s|
        ...
        s.instance_variable_set(:@_worker_killer_max_requests_verbose, verbose)
      end
    ...
    def process_client(client)
      ...
      logger.info "#{self}: worker (pid: #{Process.pid}) has #{@_worker_cur_requests} left before being killed" if @_worker_killer_max_requests_verbose
